  ...
  public void updateInputs(VisionIOInputs inputs) {
    ...
    for (PhotonPipelineResult result : camera.getAllUnreadResults()) {
      for (PhotonTrackedTarget target : result.getTargets()) {
        ...
        // Transforms the robot's pose by the camera's pose and then transforms it by the camera to
        // target translation, therefore getting the pose of the target
        Pose2d targetPose =
            robotPose
                .get()
                .plus(
                    new Transform2d(
                        robotToCamera.getX(),
                        robotToCamera.getY(),
                        new Rotation2d(robotToCamera.getRotation().getX())))
                .plus(new Transform2d(cameraToTarget, robotPose.get().times(-1).getRotation()));
      }
    }
  }
}
